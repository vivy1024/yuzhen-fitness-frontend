...
### 修复
- ⚡ **PWA加载优化：字体异步加载+路由守卫超时保护**
  - 将Google Fonts(fonts.loli.net)从render-blocking改为preload异步加载，避免CDN慢时阻塞页面渲染
  - 路由守卫中Token刷新和管理员验证添加5秒超时保护，防止API卡住导致页面永远白屏
...
- 🔐 **管理员权限验证增强**
  ...
  - 路由守卫调用后端 `/api/admin/verify` 验证管理员身份
  - 5分钟缓存机制避免频繁请求
  - 降级策略：API失败时回退到本地检查

- 🔒 **敏感数据加密存储**
  - 创建 `src/utils/crypto.ts` 加密工具（AES-GCM算法）
  - 健康数据（慢性疾病、用药情况、受伤历史）加密存储
  - 兼容未加密的旧数据自动迁移
  - 设备指纹生成唯一加密密钥

### 影响文件
- `src/api/admin/verify.ts` - 新增管理员验证API
- `src/utils/crypto.ts` - 新增加密工具函数
- `src/router/index.ts` - 路由守卫改为异步验证管理员
- `src/stores/user.ts` - 健康数据加密存储/解密读取
...
### 修复
- 🐛 **修复warmup API路径重复问题**
  - 问题：生产环境调用 `/api/api/v1/user/warmup` 返回404
  - 原因：`VITE_DAML_RAG_API_URL` 已包含 `/api` 前缀，代码中又添加了 `/api`
...
### 文档
- 📚 **API响应规范开发指南**
  - 新建完整的开发指南文档 `docs/04-开发指南/API响应规范开发指南.md`
  - 前端开发规范：Axios配置、API调用规范、Toast提示规范、字段级验证错误显示、全局错误边界
...
- 📚 **API设计规范文档更新**
  - 更新 `.kiro/steering/api-design.md` 版本至 v3.0.0
  - 补充前端处理规范（Axios拦截器、组件级错误处理、Toast提示）
  - 添加错误处理最佳实践（异常分类、环境差异化、日志记录）
  - 添加常见错误场景处理（认证错误、权限错误、验证错误、限流错误、网络错误）

### 优化
- 🎨 **响应处理规范化**
  - 所有API调用必须检查 `response.data.code`
  - 成功响应（code=200）显示成功Toast
  - 失败响应（code≠200）显示错误Toast
  - 统一使用 `response.data.msg` 作为Toast内容
...
### 优化
- 🎨 **Axios响应拦截器全面优化**
  - 添加401响应处理（清除认证+自动跳转登录+Toast）
  - 添加403响应处理（权限不足Toast）
  - 添加404响应处理（资源不存在Toast）
  - 添加422响应处理（字段级验证错误+Toast）
  - 添加429响应处理（限流Toast）
  - 添加500/502/503/504响应处理（服务器错误Toast）
  - 统一网络错误处理（网络连接失败Toast）
...
- 🎨 **Exercise API优化**
  - 使用统一的axios实例（复用auth.ts的拦截器）
  - 错误处理统一由拦截器处理

### 文档
- 📚 **创建前端响应处理审查报告**
  - 创建 `docs/06-部署运维/前端API错误处理审查报告.md`
  - 创建 `docs/06-部署运维/前端Toast使用审查报告.md`
  - 创建 `docs/06-部署运维/前端响应处理优化实施报告.md`
  - 详细记录审查结果和实施过程
...
### 修改的文件
- `src/api/auth.ts` - 优化响应拦截器
- `src/api/exercise.ts` - 使用统一axios实例
- `src/stores/auth.ts` - 添加Toast反馈
- `src/main.ts` - 添加全局错误边界
...
### 文档
- 📚 **创建7个低优先级API文档（完成100%覆盖率）**
  - 创建 `10-会员系统API.md`（含合规声明，当前禁用）
  - 创建 `11-用量管理API.md`（用量统计和限额管理）
  - 创建 `12-进度追踪API.md`（体重、体脂、围度追踪）
  - 创建 `13-用户反馈API.md`（反馈提交和管理）
  - 创建 `14-帮助中心API.md`（FAQ查询和反馈）
  - 创建 `15-管理后台API.md`（订单、反馈、用户、监控管理）
  - 创建 `16-社交登录API.md`（微信登录，部分实现）
  - 更新 `docs/05-API文档/README.md`（覆盖率50% → 100%）
  - 所有文档均包含完整的前端集成示例（TypeScript + Vue）
  - 符合需求5.4、8.4
...
### 文档
- 📚 **更新前端文档版本信息**
  - 更新 `docs/README.md` 版本号：v1.9.0 → v1.82.0
  - 更新 `docs/README.md` 更新日期：2026-01-07 → 2026-01-17
  - 更新 `docs/02-核心架构/README.md` 更新日期：2025-01-02 → 2026-01-17
  - 确保文档版本信息与CHANGELOG保持同步
  - 符合需求8.1、8.3
...
### 文档
- 📚 **创建中优先级API文档**
  - 创建 `docs/05-API文档/06-训练日志API.md`（v1.0.0）
  - 创建 `docs/05-API文档/07-个人最佳记录API.md`（v1.0.0）
  - 创建 `docs/05-API文档/08-食物库API.md`（v1.0.0）
  - 创建 `docs/05-API文档/09-动作库API.md`（v1.0.0）
  - 文档化训练日志7个API端点（记录会话、记录单组、统计等）
  - 文档化个人最佳记录6个API端点（自动更新、力量排行榜等）
  - 文档化食物库6个API端点（筛选、搜索、分类等）
  - 文档化动作库5个API端点（多维度筛选、34字段完整数据）
  - 说明闭环学习系统的数据采集流程
  - 说明RPE评分系统（1-10）和1RM估算公式（Epley）
  - 说明HTTP缓存机制（24小时）
  - 提供完整的前端集成示例（Composable + Vue组件）
  - 符合需求5.4、8.4

**API文档覆盖率**: 29% → 50%（9个已完成，9个待创建）
...
### 文档
- 📚 **补充现有API文档**
  - 更新 `docs/05-API文档/01-认证系统API.md` 到 v2.0.0
  - 更新 `docs/05-API文档/05-评分系统API.md` 到 v2.0.0
  - 添加用户档案管理API（获取、更新、统计）
  - 添加FFMI历史记录API（获取、保存）
  - 添加专家评审API
  - 添加冷启动状态API
  - 提供完整的前端集成示例
...
### 文档
- 📚 **创建高优先级API文档**
  - 创建 `docs/05-API文档/03-训练计划API.md`（v1.0.0）
  - 创建 `docs/05-API文档/04-AI聊天API.md`（v1.0.0）
  - 文档化训练计划CRUD、AI导入、进度统计等10个API端点
  - 文档化流式/非流式聊天、健康检查等3个AI API端点
  - 说明Laravel代理架构和DAML-RAG 11步工作流程
  - 提供完整的前端集成示例（TypeScript + Vue）
  - 包含SSE事件类型、错误处理、UI建议
  - 符合需求5.4、8.4
...
- 📚 **API文档同步与分析**
  - 完成前后端API文档对比分析
  - 创建 `.kiro/specs/documentation-cleanup/api-sync-analysis.md` 详细分析报告
  - 更新 `docs/05-API文档/README.md` 标注缺失文档（v1.1.0）
  - 更新根目录 `docs/05-API文档/README.md` 添加前端文档对应关系（v2.1.0）
  - 识别18个后端API模块，前端文档覆盖率17%
  - 标注高优先级待创建文档：训练计划API、AI聊天API
  - 标注待补充内容：认证系统API（用户档案）、评分系统API（专家评审）
  - 符合需求5.4、8.4
...
### 文档
- 📚 **产品文档体系完善**
  - 确认 `docs/08-产品文档/03-帮助中心FAQ.md` 已完成（v1.0.0）
  - 确认 `docs/08-产品文档/04-数据来源说明.md` 已完成（v1.0.0）
  - 更新 `docs/08-产品文档/README.md` 状态为"✅ 已完成"（v3.0.0）
  - 所有产品文档已完整，包含：
    - 品牌故事、产品介绍、帮助中心FAQ、数据来源说明、AI图标生成工具推荐
  - 符合需求5.2、8.1
...
### 文档
- 📚 **整理部署指南目录**
  - 更新 `docs/06-部署指南/zeabur-deployment-guide.md` 到 v2.0.0
  - 验证部署文档与Zeabur配置一致
  - 合并重复的部署指南（deployment.md、production-deployment-guide.md）
  - 创建 `docs/06-部署指南/README.md` 说明文档结构
  - 更新域名配置为当前生产环境：
    - 官网：`yuzhen-fitness.cn`
    - 应用PWA：`app.yuzhen-fitness.cn`
    - 后端API：`api.yuzhen-fitness.cn`
    - AI服务：`ai.yuzhen-fitness.cn`
  - 添加部署前测试清单
  - 更新环境变量配置说明
  - 符合需求5.1、5.3
...
### 新增
- 📚 **前端项目重构说明文档**
  - 创建 `docs/02-核心架构/01-系统架构/04-前端项目重构说明.md`
  - 详细说明从v2到当前版本的重构过程
  - 完整的shadcn-vue使用指南（约1000行）
  - 包含内容：
    - 重构背景和目标
    - 技术架构和组件分类
    - 设计系统（颜色、样式规范）
    - 核心功能实现（AI聊天、动作库、训练计划、会员系统）
    - 响应式设计和断点系统
    - 状态管理（Pinia Store）
    - API集成和Axios配置
    - 测试框架和覆盖率
...
### 新增
- 🎉 **公安备案信息展示**
  - `src/components/layout/Footer.vue` - 新增页脚组件，展示ICP备案和公安备案信息
  - 添加备案图标 `public/beian-icon.png`
  - 在首页、我的页面、训练页面底部显示备案信息
  - 备案号：陕ICP备2026010270号-1、陕公网安备61011602000758号

### 说明
- 符合中国大陆网站备案要求
- 备案信息显示在页面内容底部，底部导航栏上方
...
### 修复
- 🔧 **ExerciseCard组件字段更新**
  - `src/components/exercise/ExerciseCard.vue` - 移除对旧字段 `primary_muscle` 的兼容
  - 完全使用标准数组字段 `muscles_primary`
  - 配合后端MySQL字段统一（v2.81.0）

### 说明
- 三端数据库字段已完全统一为 `muscles_primary_zh` 数组格式
- 前端不再需要兼容旧的字符串字段
